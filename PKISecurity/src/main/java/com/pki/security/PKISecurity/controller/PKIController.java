...
//    }
    ...
    public ResponseEntity<CertificateTableDTO> getHostCertificate(@PathVariable("email") String email){
        try {
            CertificateTableDTO certificate = pkiService.getHostCertificate(email);
            if (certificate != null){
                return ResponseEntity.ok(certificate);
            }else{
